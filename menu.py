...
class Menu():
    def __init__(self, window_size):
        ...
        self.start_game_button = Button(
            ...
            width=100,
            height=50,
            coordinates=(self.window_size*3//4, self.window_size*3//4),
            active_color=pygame.Color('lightblue'), 
            passive_color=pygame.Color('blue'))
        ...
        while True:
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    self.inputTextBox.set_status(event.pos)
                    screen = self.start_game_button.set_status(event.pos)
                    if screen != None:
                        return screen
                if event.type == pygame.KEYDOWN and self.inputTextBox.is_active == True:
                    self.inputTextBox.get_text_after_event(event)

            screen.fill((0,0,0))
            self.draw_title(screen)
            self.inputTextBox.draw(screen)
